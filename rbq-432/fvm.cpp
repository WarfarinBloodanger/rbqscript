...
struct ObjectManager {
	...
	int get_id(string cls, string field) {
		if(!field_id[cls].count(field)) {
			error("Struct '" + cls + "' has no field called '" + field + "'.", "RuntimeError");
			return -1;
		}
	...
	Val getfield(Object object, string field) {
		cout << "getfield " << field << " of object " << object.class_name << endl;
	...
	void define_class(string cls, string super, vector<string> field_name, bool permission = false) {
	...
	Object construct(string cls, Val * args, int argc) {
		Object obj;
		obj.class_name = cls;
		reverse(args, args + argc);
		if(argc > fields[cls].size()) return obj;
		for(int i = 0; i < argc; i++) {
			obj.field[field_id[cls][fields[cls][i]]] = args[i];
		}
...
Val get_sub(Val a, Val b) {
	string str;
	if(b.type == STR_TYPE) {
		if(b.str == "type") return type_name[a.type];
		str = b.str;
		b = get_hash(b.str);
	}
	...
	if(a.type == STR_TYPE) {
		...
		return obj_val(a.obj_ref + manager.get_id(obj_heap[a.obj_ref / MAX_FIELD_CNT].class_name, str));
	}
...
Val call_builtin(string func, Val * params, int pcnt) {
	...
	if(manager.has_class(func)) {
		return obj_val(make_obj(manager.construct(func, params, pcnt)));
	}
	else if(func == "exit") {
		CNT(1);
		TYPE(0, NUM);
	...
	else if(func == "set_obj") {
		CNT(3); 
		TYPE(0, OBJ);
		TYPE(1, STR);
		cout << "Setval " << params[2].to_str() << endl;
		obj_heap[params[0].obj_ref / MAX_FIELD_CNT].field[manager.get_id(obj_heap[params[0].obj_ref / MAX_FIELD_CNT].class_name, params[1].str)] = params[2];
		return 0;
	}
	else if(func == "print") {
		...
		switch(byte) {
			...
			case CALL: {
				...
				if(rstack.top().type != FUNC_TYPE) {
					cout << "ip = " << ip << endl;
					for(int i = stkoff; i < rstack.t; i++) cout << "#" << i << "\t" << runtime_stack[i].to_str() << endl;
					error("Can not call a non-function value.");
				}
			...
			case MKCLS: {
				int cnt = rstack.pop().num;
				vstmp.clear();
				for(int i = 0; i < cnt; i++) vstmp.push_back(rstack.pop().str);
				manager.define_class(rstack.pop().str, rstack.pop().str, vstmp);
			    break;
			}
			...
			case MSET: {
				...
				int addr = v2.ref;
				Val v = rstack.pop();
				if(v2.type == REF_TYPE) set_heap(addr, v);
				else if(v2.type == OBJ_TYPE) obj_heap[v2.obj_ref / MAX_FIELD_CNT].field[v2.obj_ref % MAX_FIELD_CNT] = v;
...
#define BIND(c, rbp, code) bind_power[(c)] = (rbp)
	...
	manager.define_class("Object", "Object", tokenize("class_name super_name"), true);
}
...
vector<unsigned char> make_class(string cls, string super, vector<string> fields) {
...
vector<unsigned char> compile_statement() {
	...
	if(flag == BEGIN_LABEL) {
		...
		all_func.push_back(func_name.str);
		Token param_list = next_token();
		param_list = MIDDLE(param_list.str);
		vector<string> args = remove_all(split(param_list.str, ','), ",");
		int func_id = get_id(func_name.str);
		...
		vector<unsigned char> lint = load_func(func_name.str);
		...
		int id = func_id;
		vector<unsigned char> lint = make_class(class_name.str, super_name.str, fields);
		for(int i = 0; i < lint.size(); i++) rets.push_back(lint[i]);
