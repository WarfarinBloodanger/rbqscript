...
struct ObjectManager {
	...
	void define_class(string cls, string super, vector<string> field_name, vector<char> field_type, bool permission = false) {
		...
		for(int i = 0; i < fields[super].size(); i++) {
			...
			field_type.push_back(types[super][i]);
		}
...
Val run_byte(Val fval, int stk_offset, Val * params, int pcnt) {
	...
	for(int ip = 0; ; ip++) {
		...
		switch(byte) {
			...
			case MKCLS: {
				int cnt = rstack.pop().num;
				vstmp.clear(), vctmp.clear();
				for(int i = 0; i < cnt; i++) vctmp.push_back(rstack.pop().num), vstmp.push_back(rstack.pop().str);
				manager.define_class(rstack.pop().str, rstack.pop().str, vstmp, vctmp);
			    break;
			}
...
#define BIND(c, rbp, code) bind_power[(c)] = (rbp)
	...
	manager.define_class("Object", "Object", tokenize("class_name super_name"), vector<char>(2), true);
	manager.define_class("Class", "Object", tokenize("fields"), vector<char>(1), true);
}
...
vector<unsigned char> make_struct(string cls, string super, vector<string> fields, vector<char> type) {
...
vector<unsigned char> make_class() {
	...
	tmp = make_struct(class_name.str, super_name.str, fields, types); // Class
	for(int i = 0; i < tmp.size(); i++) rets.push_back(tmp[i]);
...
vector<unsigned char> compile_statement() {
	...
	if(flag == BEGIN_LABEL) {
		...
		int id = func_id;
		vector<unsigned char> lint = make_struct(class_name.str, super_name.str, fields, vector<char>());
		for(int i = 0; i < lint.size(); i++) rets.push_back(lint[i]);
