...
struct ObjectManager {
	...
	string super(string cls) {
		cout << "Super " << cls << " is " << superclass[cls] << endl;
...
OBJCODE NEW_CLASS = 0xac;
OBJCODE END_CLASS = 0xad;
OBJCODE USED_CLASS = 0xa0;
OBJCODE NO_SUPER_CLASS = 0xae;

OBJCODE NEW_OBJ = 0xde;
OBJCODE END_OBJ = 0xdf;
OBJCODE USED_OBJ = 0xdd;
...
void read_bytecode(string file) {
	...
	for(int i = 0; i < ccnt; i++) {
		fcin >> d;
		if(d[0] == '"') add_const(d.substr(1, d.length()));
		else add_const(atof(d.c_str()));
	}
	for(int i = 0; i < all; i++) {
		string func;
		int id;
