...
struct NativeManager {
	...
	Val load_native(string file, string funcname) {
		...
		return func_val("__native_function__" + tostr(native_funcs.size() - 1));
	}
};
...
inline bool is_builtin(string func) {
	return builtin.find(func) != builtin.end() || manager.has_class(func) || (func.length() > 19 && func.substr(0, 19) == "__native_function__");
}
...
Val call_builtin(string func, Val * params, int pcnt, int obj_id) {
	...
	else if(func.length() > 19 && func.substr(0, 19) == "__native_function__") {
