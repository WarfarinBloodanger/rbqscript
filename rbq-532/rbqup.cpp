...
string cpool[MAX_CPOOL];
int cpool_pos;

void addconst(string x) {
	cpool[cpool_pos++] = "\'" + x + "\'";
}

void addconst(double d) {
	cpool[cpool_pos++] = tostr(d);
}
...
opname[MKCLS] = "make_class";
opname[_THIS] = "load_this";
opname[PTYPE] = "check_args";
opname[RTYPE] = "check_return_val";
opname[END] = "end";
}
...
void run_byte(string func, bool of = true) {
	...
	if(of) {
		...
		cout << res["args"];
		for(int i = 0; i < reg_map[index].size(); i++) {
			printf(" %4d", reg_map[index][i]);
		}
		cout << endl;
	}
