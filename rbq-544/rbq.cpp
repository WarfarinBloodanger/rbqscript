...
const string CREDITS = "See [https://github.com/WarfarinBloodanger/rbqscript].";
const string LICENSE = "See [https://github.com/WarfarinBloodanger/rbqscript]. GNU 3.0 License is used.";
bool debug = false;
...
const int MAX_REG_CNT = 1024 * 16;
const int MAX_OBJ_CNT = 128 * 16;
...
void error(string, string = "VerifyError");
...
bool is_builtin(string);

string get_func_adj(string);

typedef int Ref;
string ref_tostr(Ref);
...
struct Val {
	...
	int extra;
	char type;
	string to_str(bool quote = false) {
		if(type == NUM_TYPE) return tostr(num);
		if(type == STR_TYPE) return (quote ? "\"" : "") + str + (quote ? "\"" : "");
		if(type == FUNC_TYPE) return "<" + get_func_adj(func) + " " + func + '>';
		if(type == REF_TYPE) return ref_tostr(ref);
...
struct ObjHeap {
	vector<Object> obj_heap;
	int size() {
		return obj_heap.size();
	}
	bool resize(int x) {
		obj_heap.resize(x);
	}
	Object& operator [] (const int & x) {
		if(x > obj_heap.size()) error("Object at " + tostr(x) + " out of bound", "OutOfBoundError");
		return obj_heap[x];
	}
} obj_heap; 
...
namespace object_controller {
	...
	bool instanceof(string cls, string cls2) {
		...
		while(cur != cls2 && cur != "Object") {
			cur = superclass[cur];
			cout << "instanceof " << cur << ' ' << cls2 << endl;
			system("pause");
		}
		return cur == cls2;
	}
	
	int identifier_id(string cls, string field) {
	...
	Val getfield(Object object, string field) {
		return object.field[identifier_id(object.class_name, field)];
	}
	
	Object setfield(Object & object, string field, Val v) {
		object.field[identifier_id(object.class_name, field)] = v;
		return object;
	}
	...
	void set_objheap(int loc, Val v) {
		...
		if(obj_id >= obj_heap.size()) obj_heap.resize(obj_id + 1);
	...
	Object construct(string cls, vector<Val> args, int argc, int id, bool chk = true) {
		Object obj;
		obj.class_name = cls;
		...
		for(int i = 0; i < types[cls].size(); i++) {
			if(types[cls][i] == CLS_FIELD) {
				...
				if(chk && !check_type(args[pos], field_type[cls][i])) {
...
namespace serializer {
	...
	Val unserialize() {
		...
		switch(read()) {
			...
			case 'f': {
				string s = read_string();
				int id = read_long_int();
				return func_val(s, id);
				break;
			}
			case UNDEF_SYMBOL: {
				return Val();
	...
	Val unserialize_obj() {
		...
		switch(read()) {
			case USED_OBJ: {
				long long x = read_long_int();
				return obj_val(obj_served[x]);
				break;
			}
			case 'm': {
				int id = get_next_obj();
			...
			case NEW_OBJ: {
				...
				read(); // END_OBJ
				obj_heap[id / MAX_FIELD_CNT] = object_controller::construct(cls_name, vals, siz, id, false);
				return obj_val(id);
				break;
			}
			default: {
...
struct MD5 {
	...
	static char * encode(string s) {
		unsigned char e[32];
		memset(buffer, 0, sizeof(buffer));
		MD5_CTX md5;
		MD5Init(&md5);              
		MD5Update(&md5, (unsigned char *) s.c_str(), s.length());
		MD5Final(&md5, (unsigned char *) e);
...
struct Regex {
	static string match_first(string text, string pattern) {
		regex p(pattern); smatch r;
		if(regex_search(text, r, p)) {
			return r.str();
		}
		else return "";
	}
	static vector<Val> match_all(string text, string pattern) {
		regex p(pattern); smatch r;
		vector<Val> ret;
		for(
			sregex_iterator it(text.begin(), text.end(), p), end_it;
			it != end_it;
			++it
		) {
...
namespace native_controller {
	...
	Val load_native(string file, string funcname) {
	...
	#ifdef _WIN32
		HINSTANCE h = LoadLibrary(file.c_str());
		if(h == NULL) return -1;
		func = (RbqNativeFunc) GetProcAddress(h, funcname.c_str());
	#elif defined(linux)
		void * handle = dlopen(file.c_str(), RTLD_LAZY);
		if(!handle) return -1;
		func = (RbqNativeFunc) dlsym(handle, funcname.c_str());
		char * err = dlerror();
		if(err != NULL) {
			dlclose(handle);
			return (string)err;
		...
		string fname = file + "$" + tostr(native_funcs.size() - 1);
		natives.insert(fname);
		return func_val(fname);
	}
	bool is_native(string f) {
...
vector<Val> heap;
const int MAX_HEAP_SIZE = ARR_GROW * MAX_ARR_CNT; 
bool used_loc[MAX_ARR_CNT];

int get_len(Val);
set<int> tsref;
string ref_tostr(Ref ref) {
	if(tsref.find(ref) != tsref.end()) return "<Location at " + tostr(ref) + ">";
	tsref.insert(ref);
	int loc = ref / ARR_GROW;
	stringstream ss("");
	ss << "{";
	int len = get_len(ref_val(ref));
	for (int i = 0; i < len; i++) {
		ss << get_heap(ref + i).to_str(true);
		if(i != len - 1) ss << ", ";
	}
	ss << "}";
	tsref.erase(ref);
...
Val get_heap(int loc) {
	if(loc < 0 || loc >= heap.size()) error("heap index '" + tostr(loc) + "' out of bound", "RuntimeError");
...
void set_heap(int loc, Val v) {
	...
	if(heap.size() <= loc) heap.resize(loc + 1);
	heap[loc] = v;
}

string get_name(int);
void local_vm_set_reg(int pos, Val v) {
...
string get_name(int);
Val load(int pos) {
	if(!not_check && pos > MAX_LOCAL_CNT) error("Variable " + get_name(pos) + " is not declared nor initialized", "NameError");
	for(int i = itop; i >= 1; i--) {
		Val v = reg.r[i * MAX_LOCAL_CNT + pos];
		if(v.type != UNDEFINED_TYPE) return v;
	}
	error("Variable " + get_name(pos) + " is not declared nor initialized", "NameError");
}
...
int get_next_obj() {
	...
	if(x >= obj_heap.size()) obj_heap.resize(x + 1);
...
int get_len(Val p) {
	...
	if(p.type == STR_TYPE) {
		...
		for(int i = 0; i < ARR_GROW; i++) {
			if(p.ref + i >= heap.size() || heap[(int)p.ref + i].type == UNDEFINED_TYPE) break;
			len++;
		}
		return len;
	}
...
Val call_builtin(string func, vector<Val> params, int pcnt, int obj_id) {
	...
	CHECK_MATH("abs", fabs)
		...
		TYPE(0, NUM);
		TYPE(1, NUM);
		return make_range(params[0].num, params[1].num);
	}
	else if(func == "_md5") {
		CNT(1);
	...
	else if(func == "read_eof") {
		CNT(0);
		return cin.eof();
	}
	else if(func == "_regex_match") {
		CNT(2);
		TYPE(0, STR);
		TYPE(1, STR);
		return Regex::match_first(params[0].str, params[1].str);
	}
	else if(func == "_regex_match_all") {
		CNT(2);
		TYPE(0, STR);
		TYPE(1, STR);
		return make_array(Regex::match_all(params[0].str, params[1].str));
	}
	else if(func == "_abort") {
		CNT(2);
		error(params[0].to_str(), params[1].to_str());
		return Val();
	}
}
int x;
...
try {
	...
	call_stack.push_back(fval.func);
	if(is_builtin(func)) {
		Val v = call_builtin(func, params, pcnt, fval.obj_ref);
		call_stack.pop_back();
		return v;
	}
	if(call_pause) {
	...
	for(int i = 0; i < pcnt; i++) {
		local_vm_set_reg(reg_map[index][i], params[i]);
	}
	unsigned char byte;
	...
	for(int ip = 0; ; ip++) {
		...
		switch(byte) {
			...
			case STOREC: {
				int l_h = byte_map[index][++ip];
				vm_set_reg(l_h, rstack.pop());
				break;
			}
			case LSTOREC: {
				int l_h = byte_map[index][++ip];
				local_vm_set_reg(l_h, rstack.pop());
				break;
			}
			case CALL: {
			...
			case LSTORE: {
				...
				int l_l = byte_map[index][++ip];
				int ldc = l_l + l_h * 256;
				local_vm_set_reg(ldc, rstack.pop());
			    break;
			}
			case MGET: {
				Val v = rstack.pop();
...
string trim(string s) {
    ...
    while(x < s.length() && (isspace(s[x]) || s[x] == 0)) x++;
    while(y >= x && (isspace(s[y]) || s[y] == 0)) y--;
...
string get_name(int id) {
	for(map<string, int>::iterator it = var_id.begin(); it != var_id.end(); it++) {
		...
		int idx = (*it).second;
		if(idx == id) {
			if(!name.length()) error("empty identifier", "SyntaxError");
			if(isdigit(name[0])) {
				error("identifier should not begin with numbers: '" + name + "'", "SyntaxError");
			}
			else {
				for(int i = 0; i < name.length(); i++) {
					if(!check_char(name[i])) {
						error("invalid token '" + name + "'", "SyntaxError");
					}
				}
			}
			return name;
...
void check_name(string v) {
	string r = "";
	bool bad = false;
	bool is_cl = false;
	if(!v.length()) return;
	if(v[0] >= 'A' && v[0] <= 'Z') is_cl = true;
	r += v[0];
	for(int i = 1; i < v.length(); i++) {
		if(!is_cl && v[i] >= 'A' && v[i] <= 'Z') {
			r += "_", r += tolower(v[i]), bad = true;
		}
		else r += v[i];
	}
	if(bad) warn("please use '" + r + "' to replace '" + v + "' for identifier", "VarNameWarning");
}
int varcnt;
int identifier_id(string var) {
	if(var_id.find(var) == var_id.end()) var_id[var] = varcnt, varcnt++;
	if(strict_name) check_name(var);
	return var_id[var];
}
...
#define BIND(c, rbp, code) bind_power[(c)] = (rbp)
    ...
    #define BUILTIN(a) vm_set_reg(identifier_id(a), func_val(a)), builtin.insert(a)
    #define ADDCONST(a, x) vm_set_reg(identifier_id(a), x), protect.insert(identifier_id(a))
	...
	ADDCONST("true", Val(1));
	ADDCONST("false", Val(0));
	ADDCONST("null", null_val());
	ADDCONST("math_pi", Val(3.1415926535897932384));
	ADDCONST("license", LICENSE);
	ADDCONST("credits", CREDITS);
	ADDCONST("help", HELP);
	ADDCONST("copyright", COPYRIGHT);
	...
	BUILTIN("_md5");
	BUILTIN("typeof");
	BUILTIN("_regex_match");
	BUILTIN("_regex_match_all");
	BUILTIN("_abort");
}
...
vector<unsigned char> make_class() {
	...
	int id = identifier_id(class_name.str);
...
vector<unsigned char> make_function(string & fname, string prefix = "") {
	...
	for(int i = 0; i < args.size(); i++) regs.push_back(identifier_id(args[i]));
...
vector<unsigned char> compile_statement() {
	...
	if(flag == BEGIN_LABEL) {
		...
		int var_id = identifier_id(var);
		int read_id = identifier_id(real_index);
		vector<unsigned char> fret = compile_expression(first);
		for(int i = 0; i < fret.size(); i++) rets.push_back(fret[i]);
		if(short_op && var_id >= 0 && var_id <= 0xff) {
			rets.push_back(LSTOREC);
		...
		int func_id = identifier_id(class_name.str);
		int id = func_id;
...
vector<unsigned char> make_lambda(string expr, string param_list) {
	...
	int func_id = identifier_id(func_name);
...
void read_file_source(string file) {
	...
	while(getline(fcin, str)) {
		...
		if(tokens.size() && tokens[0] == "include") {
			string f = "";
...
void start_compile() {
	string str;
	main_id = identifier_id(LAUNCH_BLOCK);
	int line = 0;
	if (!all_source.str().length()) return;
	while(getline(all_source, str)) {
		...
		if(tokens.size() && tokens[tokens.size() - 1] != BEGIN_LABEL && tokens[tokens.size() - 1] != END_LABEL) {
...
void debug_output() {
	for(int f = 0; f < all_func.size(); f++) {
		...
		int id = identifier_id(all_func[f]);
		cout << "\tid " << id << endl;
